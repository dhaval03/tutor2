// Image Manager
    ...
    if(side && side == 'rtl') {
		...
		$('#pts-button-image').on('click', function() {
			...
			$.ajax({
				url: 'index.php?route=extension/purpletree_multivendor/multivendor/common/filemanager&target=' + $element.parent().find('input').attr('id') + '&thumb=' + $element.attr('id')+'&seller_id=' + seller_id +'&seller_name=' + seller_name,
				dataType: 'html',
				beforeSend: function() {
