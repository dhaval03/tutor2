...
class PurpletreeLatestsellerProducts extends \Opencart\System\Engine\Controller {
		public function index() {
			...
			$this->load->model('extension/purpletree_multivendor/module/latestseller_products');
			
			
			$productss = $this->model_extension_purpletree_multivendor_module_latestseller_products->getLatest();
					...
					//if($count < 8) {
						...
						if ($product_info) {
							...
							if (strpos($this->config->get('config_template'), 'journal2') === 0){
								...
								$additional_imagess = $this->model_catalog_product->getImages($product['product_id']);               
						...
						$data['show_seller_address'] = $this->config->get('module_purpletree_multivendor_show_seller_address');
							...
							'quick_order'       => $this->url->link('extension/purpletree_multivendor/multivendor/quick_order', '&product_id='.$product_info['product_id'].'&language=' . $this->config->get('config_language'), true),
			...
			if ($data['products']) {
			return $this->load->view('extension/purpletree_multivendor/module/purpletree_latestseller_products', $data);
		} else {
			return '';
		}
		}
