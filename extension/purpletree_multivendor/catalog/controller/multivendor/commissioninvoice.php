...
class Commissioninvoice extends \Opencart\System\Engine\Controller{
		...
		public function index(){
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice','language=' . $this->config->get('config_language'), true);
			...
			'href' => $this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice',$url, true)
			);
			...
			'start'                => ((int)$page - 1) * (int)$this->config->get('config_pagination_admin'),
			...
			if(!empty($seller_commissions)) {
				foreach($seller_commissions as $invocee) {
					...
					'view' 			=> $this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice|view', '&commision_view_id='.$invocee['id'].'&language=' . $this->config->get('config_language'), true)
					);
				}
			...
			'page'  => (int)$page,
			'limit' => (int)$this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice',$url . '&page={page}'.'language=' . $this->config->get('config_language'), true)
		]);

		$data['results'] = sprintf($this->language->get('text_pagination'), ((int)$total_commissions) ? (((int)$page - 1) * (int)$this->config->get('config_pagination_admin')) + 1 : 0, ((((int)$page - 1) * (int)$this->config->get('config_pagination_admin')) > ((int)$total_commissions - (int)$this->config->get('config_pagination_admin'))) ? (int)$total_commissions : ((((int)$page - 1) * (int)$this->config->get('config_pagination_admin')) + (int)$this->config->get('config_pagination_admin')), (int)$total_commissions, ceil((int)$total_commissions / (int)$this->config->get('config_pagination_admin')));
		...
		public function view(){
			...
			if(!$this->ptsValidateSeller) {
				...
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice','language=' . $this->config->get('config_language'), true));
			}
			$this->load->model('extension/purpletree_multivendor/multivendor/dashboard');
			$this->model_extension_purpletree_multivendor_multivendor_dashboard->checkSellerApproval();
			...
			if(!empty($commisionss)) {
				foreach ($commisionss as $commisionid) {
					...
					$data['sellerdetails']['store_email']  = $this->model_extension_purpletree_multivendor_multivendor_vendor->getCustomerEmailId($commisionid['seller_id']);			
					...
					$order_info = $this->model_extension_purpletree_multivendor_multivendor_commissioninvoice->getanyOrder(trim($order_id));
