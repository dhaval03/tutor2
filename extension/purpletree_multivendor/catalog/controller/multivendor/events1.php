...
class Events extends \Opencart\System\Engine\Controller {
	...
	public function afterHeaderView(&$route, &$data, &$output) {
			...
			$data['browse_seller_link'] = $this->url->link('extension/purpletree_multivendor/multivendor/sellers');
			$data['seller_register_link'] = $this->url->link('extension/purpletree_multivendor/multivendor/sellerlogin');
			///// seller panel/////
			   ...
			   $store_detail = $this->model_extension_purpletree_multivendor_multivendor_vendor->isSeller($this->customer->getId());	
				if($store_detail){
						if($store_detail['is_removed']==1){
							$data['seller_panel_link'] = $this->url->link('extension/purpletree_multivendor/multivendor/sellerstore/becomeseller', '', true);
						} else {
						$stores=array();
						...
						if($store_detail['store_status']==1 && in_array($this->config->get('config_store_id'),$stores)){
							$data['seller_panel_link'] = $this->url->link('extension/purpletree_multivendor/multivendor/dashboardicons', '', true);
						} elseif(!in_array($this->config->get('config_store_id'),$stores)){
						  $data['seller_panel_link'] = $this->url->link('extension/purpletree_multivendor/multivendor/sellerlogin');
