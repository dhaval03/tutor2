...
class Shipping extends \Opencart\System\Engine\Controller {
		...
		public function add() {
			...
			if (!$validateSeller) {
				$this->load->language('extension/purpletree_multivendor/multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
			$this->load->language('extension/purpletree_multivendor/multivendor/shipping');
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()) ) {
				...
				$this->model_extension_purpletree_multivendor_multivendor_shipping->addShipping($this->request->post);
		...
		public function edit() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				$this->model_extension_purpletree_multivendor_multivendor_shipping->editShipping($this->request->get['shipping_id'], $this->request->post);
		...
		protected function getList() {
		  $this->document->addStyle('../extension/purpletree_multivendor/admin/view/javascript/purpletreecss/commonstylesheet.css');
			...
			'href' => $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . $url, true)
			);
			
			$data['add'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping|add', 'user_token=' . $this->session->data['user_token'] . $url, true);
			$data['delete'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping|delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
			...
			foreach ($results as $result) {
				...
				if(($result['seller_id'] != 0) && ($result['seller_id'] != -1) ) {
					$seller = $this->model_extension_purpletree_multivendor_multivendor_shipping->getSellersName($result['seller_id']);
					if($seller) {
						$name = $seller['name'];
					}
				}else{
				...
				'deletes'           => $this->url->link('extension/purpletree_multivendor/multivendor/shipping|deletes', 'user_token=' . $this->session->data['user_token'] . '&shipping_id=' . $result['id'] . $url, true),
				'edit'           => $this->url->link('extension/purpletree_multivendor/multivendor/shipping|edit', 'user_token=' . $this->session->data['user_token'] . '&shipping_id=' . $result['id'] . $url, true)
				);
			}
			...
			$data['sort_name'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
			$data['sort_shipping_country'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=cu.name' . $url, true);
			$data['sort_zip_from'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=pvs.zipcode_from' . $url, true);
			$data['sort_zip_to'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=pvs.zipcode_to' . $url, true);
			$data['sort_price'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=pvs.shipping_price' . $url, true);
			$data['sort_weight_from'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=pvs.weight_from' . $url, true);
			$data['sort_weight_to'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . '&sort=pvs.weight_to' . $url, true);
			$url = '';
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true)
		]);
			...
			$data['sort'] = $sort;
			$data['order'] = $order;
			$data['url'] = $this->url->link('extension/purpletree_multivendor/multivendor/bulkshippingupload', 'user_token=' . $this->session->data['user_token'], true);
			$this->load->model('localisation/country');
			$data['countries'] = $this->model_localisation_country->getCountries();		
			$data['header'] = $this->load->controller('common/header');
			$data['column_left'] = $this->load->controller('common/column_left');
			$data['footer'] = $this->load->controller('common/footer');
			$this->response->setOutput($this->load->view('extension/purpletree_multivendor/multivendor/shipping_list', $data));
		}
		...
		protected function getForm() {
			...
			if (!isset($this->request->get['shipping_id'])) {
				$data['action'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping|add', 'user_token=' . $this->session->data['user_token'] . $url, true);
				} else {
				$data['action'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping|edit', 'user_token=' . $this->session->data['user_token'] . '&shipping_id=' . $this->request->get['shipping_id'] . $url, true);
			}	
			
			$data['cancel'] = $this->url->link('extension/purpletree_multivendor/multivendor/shipping', 'user_token=' . $this->session->data['user_token'] . $url, true);
			if (isset($this->request->get['shipping_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				$shipping_info = $this->model_extension_purpletree_multivendor_multivendor_shipping->getSellerShipping($this->request->get['shipping_id']);
			}
			...
			$this->response->setOutput($this->load->view('extension/purpletree_multivendor/multivendor/shipping_form', $data));
		}
		
		protected function validateForm() {
			
			/* if (strlen($this->request->post['filter_name']) == '') {
			...
			if(strlen($this->request->post['price']) < 1){
			...
			if(strlen($this->request->post['weight_from']) < 1){
			...
			if(strlen($this->request->post['weight_to'] ) < 1){
		...
		protected function validateDelete() {
			if (!$this->user->hasPermission('modify', 'extension/purpletree_multivendor/multivendor/shipping')) {
		...
		public function autocompleteseller() {
			...
			$this->load->model('extension/purpletree_multivendor/multivendor/shipping');
			$results = $this->model_extension_purpletree_multivendor_multivendor_shipping->getSellers($filter_data);
			foreach ($results as $result) {
				$json[] = array(
