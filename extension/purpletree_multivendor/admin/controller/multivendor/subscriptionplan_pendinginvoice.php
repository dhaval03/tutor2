...
class SubscriptionplanPendinginvoice extends \Opencart\System\Engine\Controller {
		private $error = array();		
		public function index() {
			...
			$this->load->language('extension/purpletree_multivendor/multivendor/subscriptionplan_pendinginvoice');
			
			$this->document->setTitle($this->language->get('heading_title'));
			
			$this->load->model('extension/purpletree_multivendor/multivendor/subscriptionplanpendinginvoice');
		...
		$data['helpimage'] = HTTP_CATALOG . '/extension/purpletree_multivendor/admin/view/image/help.png';
			...
			'href' => $this->url->link('extension/purpletree_multivendor/multivendor/subscriptionplan_pendinginvoice', 'user_token=' . $this->session->data['user_token'] . $url, true)
			);
			...
			$subs_invoice_total=$this->model_extension_purpletree_multivendor_multivendor_subscriptionplanpendinginvoice->getTotalSellerInvoice($filter_data);
			$results = $this->model_extension_purpletree_multivendor_multivendor_subscriptionplanpendinginvoice->getSellerInvoiceList($filter_data);
			if(!empty($results)){
				foreach ($results as $result) {
					
					/* $invoice_status = $this->model_extension_purpletree_multivendor_multivendor_subscriptionplanpendinginvoice->getInvoiceStatus($result['invoice_id']); */
					$subscription_status='';
					$s_status=$this->model_extension_purpletree_multivendor_multivendor_subscriptionplanpendinginvoice->getSubscriptionBySellerId($result['seller_id']);
					if($s_status==1){
			...
			// echo "<Pre>";
			// print_r($this->config->get('config_pagination_admin'));
			// die;
			
			$data['pagination'] = $this->load->controller('common/pagination', [
			'total' => $subs_invoice_total,
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/purpletree_multivendor/multivendor/subscriptionplan_pendinginvoice', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true)
		]);
			
			$data['results'] = sprintf($this->language->get('text_pagination'), ($subs_invoice_total) ? (($page - 1) * $this->config->get('config_pagination_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_pagination_admin')) > ($subs_invoice_total - $this->config->get('config_pagination_admin'))) ? $subs_invoice_total : ((($page - 1) * $this->config->get('config_pagination_admin')) + $this->config->get('config_pagination_admin')), $subs_invoice_total, ceil($subs_invoice_total / $this->config->get('config_pagination_admin'))); 
			...
			$this->response->setOutput($this->load->view('extension/purpletree_multivendor/multivendor/subscriptionplan_pendinginvoice', $data));
		}
