...
class Sellercommission extends \Opencart\System\Engine\Controller {
		...
		public function index(){
			...
			'href' => $this->url->link('extension/purpletree_multivendor/multivendor/sellercommission', 'user_token=' . $this->session->data['user_token'] . $url, true)
			);
			...
			$seller_commissions1 = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getCommissions($filter_data);
			$seller_commissions111 = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getCommissions111($filter_data);
			...
			if($seller_commissions){
				foreach($seller_commissions as $seller_commission){
					//$seller_commissions2 = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getCommissions11($seller_commission1['order_id']);
					//foreach($seller_commissions2 as $seller_commission){
						...
						'store_url' => $this->url->link('extension/purpletree_multivendor/multivendor/stores|edit&store_id='.$seller_commission['store_id'], 'user_token=' . $this->session->data['user_token'], true),
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('extension/purpletree_multivendor/multivendor/sellercommission', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true)
			]);
			...
			$data['seller_stores'] = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getSellerstore();
			...
			$data['commission_invoice'] = $this->url->link('extension/purpletree_multivendor/multivendor/sellercommission|generate', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		public function generate(){
			...
			if (!$validateSeller) {
				$this->load->language('extension/purpletree_multivendor/multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$this->response->redirect($this->url->link('extension/purpletree_multivendor/multivendor/sellercommission', 'user_token=' . $this->session->data['user_token'], true));
			}
			$this->load->language('extension/purpletree_multivendor/multivendor/sellercommission');
			...
			foreach($this->request->post['selected'] as $key => $value){
			 $commission_invoice_check = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->checkCommissionInvoice($value,$this->request->get['seller_id']);
			...
			if (isset($this->request->post['selected'])) {
				...
				try {
					...
					if(!empty($commisionss)) {
						foreach ($commisionss as $commisionid => $order_id) {
							$commisionssss = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getCommissionsforinvoide($commisionid);
						...
						foreach($uniqueSoId as $vvvv){
							$total_price[$vvvv['seller_id']][]= $this->model_extension_purpletree_multivendor_multivendor_commissioninvoice->getCommissionTotal($vvvv['order_id'],$vvvv['seller_id']);
							
							$coupon_amount[$vvvv['seller_id']][]= $this->model_extension_purpletree_multivendor_multivendor_commissioninvoice->getCouponAmount($vvvv['order_id'],$vvvv['seller_id']);
						}
						...
						if(!empty($t_comm)){
							foreach($t_comm as $seller_idd=>$seller_commm){
								...
								$linkid[$seller_idd] = $this->model_extension_purpletree_multivendor_multivendor_commissioninvoice->savelinkid($total_price,$seller_commm,$total_pay_amount,$coupon_amt);
							}
						}
						foreach ($commisionss as $commisionid => $order_id) {
							$commisionsss = $this->model_extension_purpletree_multivendor_multivendor_sellercommission->getCommissionsforinvoide($commisionid);
							...
							if(!empty($commisionsss)) {
								if($commisionsss['order_id'] == $order_id && $commisionsss['invoice_status'] == 0) {
									...
									$this->model_extension_purpletree_multivendor_multivendor_commissioninvoice->saveCommisionInvoice($commisionsss,$linkidd);
				...
				if(!empty($linkid)){
					$this->response->redirect($this->url->link('extension/purpletree_multivendor/multivendor/commissioninvoice|commissionInvoice', 'user_token=' . $this->session->data['user_token'], true));
				}
			}
		}else{
		  ...
		  $this->session->data['generated_invoice_order_id'] = $generated_invoice_order_id;
			...
			$this->response->redirect($this->url->link('extension/purpletree_multivendor/multivendor/sellercommission', 'user_token=' . $this->session->data['user_token'].$seller_id , true));
