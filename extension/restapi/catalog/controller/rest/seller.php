...
//            );
        ...
        if (empty($this->json['error'])) {
            ...
            $user = $this->model_seller_customer->getCustomer($this->customer->getId());

            // Custom Fields
            $this->load->model('seller/custom_field');
            $this->load->model('seller/reward');

            $custom_fields = $this->model_seller_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
           ...
           if ($this->opencartVersion < 2100) {
                $user['custom_field'] = unserialize($user['custom_field']);
            } else {
                $user['custom_field'] = json_decode($user['custom_field'], true);
            }

            $reward_total = $this->model_seller_reward->getTotalRewards();
            ...
            $results = $this->model_seller_reward->getRewards($filter_data);
    ...
    public function saveSeller($post)
    {
        if (!$this->customer->isLogged()) {
            ...
            if ($this->validate($post)) {
                $customer = $this->model_seller_customer->getCustomer($this->customer->getId());
                ...
                foreach ($custom_fields as $custom_field) {
                    if ($custom_field['location'] == 'seller') {
                ...
                $this->model_seller_customer->editCustomer($this->customer->getId(), $post);
            } else {
    ...
    protected function validate($post)
    {
        ...
        if (isset($post['email'])) {
            ...
            if (!isset($post['email']) || ($this->customer->getEmail() != $post['email']) && $this->model_seller_customer->getTotalCustomersByEmail($post['email'])) {
        ...
        foreach ($custom_fields as $custom_field) {
            if($this->opencartVersion < 2300) {
                if (($custom_field['location'] == 'seller') && $custom_field['required'] && empty($post['custom_field'][$custom_field['custom_field_id']])) {
    ...
    public function changePassword($post)
    {
        ...
        $this->load->language('seller/password');
        ...
        if (!$this->customer->isLogged()) {
            ...
            if (empty($this->json["error"])) {
                ...
                $this->model_seller_customer->editPassword($this->customer->getEmail(), $post['password']);
    ...
    public function customfield()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $this->json['data'] = $this->model_seller_custom_field->getCustomFields($customer_group_id);
...
//                );
    ...
    private function getAddress($id)
    {
        ...
        $address = $this->model_seller_address->getAddress($id);
        if (!empty($address)) {
            // Custom Fields
            $this->load->model('seller/custom_field');
    ...
    private function listAddress()
    {
        ...
        $data['addresses'] = $this->model_seller_address->getAddresses();
...
//            );
    ...
    private function addAddress($data)
    {
        ...
        if (empty($this->json["error"])) {
            ...
            $id = $this->model_seller_address->addAddress($this->customer->getId(), $data);
    ...
    private function editAddress($id, $data)
    {
        ...
        if (!empty($address)) {
            ...
            $address['custom_fields'] = $this->model_seller_custom_field->getCustomFields($this->config->get('config_customer_group_id'));
        }
        ...
        if (empty($this->json["error"])) {
            $this->model_seller_address->editAddress($id, $data);
        } else {
            $this->statusCode = 400;
        }
    }
    ...
    private function deleteAddress($id)
    {
        ...
        $this->load->model('seller/address');
        $this->model_seller_address->deleteAddress($id);
    }
    ...
    public function subscribe($subscribe)
    {
        ...
        if (!$this->customer->isLogged()) {
            ...
            $this->load->model('seller/customer');
            $this->model_seller_customer->editNewsletter($subscribe);
        }
    }
    ...
    public function loadTransactions()
    {
        ...
        if (!$this->customer->isLogged()) {
            ...
            $results = $this->model_seller_transaction->getTransactions($filter);
...
//                );
    ...
    public function addVoucher($post)
    {
        ...
        $this->load->language('seller/voucher');
...
//                );
    ...
    private function getDownload($id)
    {
        ...
        $download_info = $this->model_seller_download->getDownload($id);
            ...
            if (!headers_sent()) {
                if (file_exists($file)) {
                    ...
                    if ($this->opencartVersion >= 3100) {
                        $this->model_seller_download->addDownloadReport($id, $this->request->server['REMOTE_ADDR']);
                    }
    ...
    private function listDownloads()
    {
        ...
        $results = $this->model_seller_download->getDownloads($start, $limit);
        ...
        foreach ($results as $result) {
            if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
                ...
                $download_info = $this->model_seller_download->getDownload($result['download_id']);
                ...
                $data['downloads'][] = array(
                    ...
                    'href'       => $this->url->link('seller/download/download', 'download_id=' . $result['download_id'], true),
                    'download_url' => $this->url->link('rest/seller/downloads', 'id=' . $result['download_id'], true)
                );
            }
        }
        ...
        if($this->includeMeta) {
            ...
            $total = $this->model_seller_download->getTotalDownloads();
...
//            );
    ...
    public function recurrings()
    {
        ...
        $this->load->language('seller/recurring');
    ...
    private function listRecurrings()
    {
        ...
        $results = $this->model_seller_recurring->getOrderRecurrings($start, $limit);
        ...
        if($this->includeMeta) {
            ...
            $total = $this->model_seller_recurring->getTotalOrderRecurrings();
            $this->response->addHeader('X-Total-Count: ' . (int)$total);
            $this->response->addHeader('X-Pagination-Limit: '.(int)$limit);
            $this->response->addHeader('X-Pagination-Page: '.(int)$page);
//            $data = $this->json['data'];
...
//            );
    ...
    private function getRecurring($id)
    {
        ...
        $recurring_info = $this->model_seller_recurring->getOrderRecurring($id);
        ...
        if ($recurring_info) {
            ...
            $results = $this->model_seller_recurring->getOrderRecurringTransactions($id);
